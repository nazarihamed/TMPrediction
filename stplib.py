...
class Stp(app_manager.RyuApp):
    ...
    def _register_bridge(self, dp):
        ...
        self.logger.debug('Join as stp bridge.', extra=dpid_str)
        try:
    ...
    def _unregister_bridge(self, dp_id):
        if dp_id in self.bridge_list:
            ...
            self.logger.debug('Leave stp bridge.',
        ...
        if dp.id in self.bridge_list:
            ...
            if reason is dp.ofproto.OFPPR_ADD:
                ...
                if bridge.ports_state[port.port_no] == port.state:
                    ...
                    return
                if link_down_flg:
                    self.logger.debug('[port=%d] Link down.',
                                     port.port_no, extra=dpid_str)
                    bridge.link_down(port)
                else:
                    self.logger.debug('[port=%d] Link up.',
                                     port.port_no, extra=dpid_str)
                    bridge.link_up(port)
...
class Bridge(object):
    ...
    def packet_in_handler(self, msg):
        ...
        if bpdu.ConfigurationBPDUs in pkt:
            ...
            if rcv_info is SUPERIOR:
                self.logger.debug('[port=%d] Receive superior BPDU.',
    ...
    def recalculate_spanning_tree(self, init=True):
        ...
        if init:
            self.logger.debug('Root bridge.', extra=self.dpid_str)
    ...
    def _spanning_tree_algorithm(self):
        ...
        if root_port is None:
            ...
            self.logger.debug('Non root bridge.', extra=self.dpid_str)
...
class Port(object):
    ...
    def _state_machine(self):
        ...
        while True:
            self.logger.debug('[port=%d] %s / %s', self.ofport.port_no,
        ...
        while True:
            ...
            try:
                ...
                if t is not timeout:
                    err_msg = 'Internal error. Not my timeout.'
                    raise RyuException(msg=err_msg)
                self.logger.debug('[port=%d] Wait BPDU timer is exceeded.',
                                 self.ofport.port_no, extra=self.dpid_str)
                time_exceed = True
            finally:
                timeout.cancel()
